...
from datasets.PGNN_simulator_time import PGNNDataset_simulator, PGNNDataset_new
from model.PGNN_simulator_time import SimulatorModel
...
def train_epoch(
    ...
    for iteration, batch in enumerate(train_data_loader, 1):
        ...
        loss_cpt = model.loss_cpt(prediction, target)
        loss_equation = model.loss_equation(cpts, bzmesh, prediction)
        loss = loss_cpt + loss_equation
...
def train(args):
    ...
    if args.data_fname is not None:
        ...
        dataset = PGNNDataset_simulator(data_fname)
...
def evaluation(args):
    ...
    loss = checkpoint["loss"]
    model.eval()
    print("Evaluation checkpoint: ", checkpoint_fname)
